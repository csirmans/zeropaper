...
- **Corbis MCP for adjacent literatures.** You have the `corbis` skill loaded — see it for full usage. Read `process_log/corbis_status.json` first and follow the shared state protocol: check `process_log/corbis_cache.jsonl`, reserve budget, mark success/failure with the same scope via `--stage`, and cache reusable hits with `code/utils/corbis/state.py`. Use budget scope `stage0_literature` when validating the Stage 0 gap and `stage3_implication:<implication_id>` when lit-checking an implication (for example, `stage3_implication:impl_2`; each implication gets its own 4-call budget). Corbis auth is OAuth/client-managed, so the preflight records `available: null`; run a Corbis pass alongside OpenAlex when the runtime exposes the tools and budget allows it. Use the `search` capability for each adjacent angle and selective `top_cited` calls to surface foundational papers. For closest-competitor identification, `search` ranked by citation count is the right tool. Resolve capabilities via `corbis_status.json["capabilities"]`; treat the map as default unverified tool names and fall back cleanly if auth/tool access fails. If Corbis is disabled for this stage or budget is exhausted, continue with OpenAlex + WebSearch.
