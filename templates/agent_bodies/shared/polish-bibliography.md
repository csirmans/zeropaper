...
1. **Look up the cited paper.** Start with `output/bib_verification.jsonl` if present: skip FABRICATED cite keys, reuse captured WebSearch URLs, and treat Corbis-confirmed entries as already validated identities. Then check `process_log/corbis_cache.jsonl` by DOI/title/OpenAlex ID/Corbis ID before any new lookup. Try Corbis first for load-bearing finance/econ citations when the paper is plausibly in the Corbis corpus — Corbis returns full text on many papers, which gives you a much stronger basis for the prose-claim audit than abstracts alone. Corbis auth is OAuth/client-managed, so the preflight records `available: null`; try Corbis only if the runtime exposes the tools, the `polish_bibliography` budget allows it, and the stage is not disabled. Fall back cleanly to OpenAlex when Corbis is unavailable, budget-exhausted, disabled, or returns no hit. You need at minimum the abstract; the introduction or first section is even better.
...
- For cites with cached Corbis/OpenAlex metadata, reuse the cached identity and abstract/snippet before spending a new lookup.
...
- **Hard cap: 50 lookups per run (Corbis + OpenAlex combined), with at most 30 Corbis calls.** Track the count yourself; stop after the 50th successful lookup regardless of how many citations remain unaudited and note the shortfall in your report. Cache hits do not count as new lookups. The cap is total, not per-source — a Corbis hit and a falling-back OpenAlex lookup both count. For papers with more than 50 cites, prioritize in this order: (a) cites immediately preceded by "shows," "proves," "documents," "finds," "establishes"; (b) cites contrasted with the paper's own claim ("unlike X," "departing from X," "in contrast to X"); (c) all cites in the introduction; (d) cites in propositions/discussion sections. Skip pure literature-list cites in related-work paragraphs (clusters of 3+ cites in one parenthetical). Record skipped cites with a one-line reason in a `## Unaudited (cap reached)` section of your report so the orchestrator knows what was not checked.

## Tools

- **Corbis MCP** (skill `corbis`) — preferred for finance/econ papers. Read `process_log/corbis_status.json` first and follow the shared state protocol: check cache, reserve from budget scope `polish_bibliography`, mark success with `--stage polish_bibliography`, mark failures with the same stage, and cache reusable compact metadata with `code/utils/corbis/state.py`. Use the `search` capability to find the paper, then `batch_fetch` (or per-paper details) to get the abstract and (when available) full text when the runtime exposes those tools. Resolve every tool via `corbis_status["capabilities"]`; treat the map as default unverified tool names and fall back cleanly if auth/tool access fails, budget is exhausted, or the stage is disabled.
