...
1. **Look up the cited paper.** Try Corbis first when the paper is plausibly in the finance/econ corpus — Corbis returns full text on many papers, which gives you a much stronger basis for the prose-claim audit than abstracts alone. Corbis auth is OAuth/client-managed, so the preflight records `available: null`; try Corbis only if the runtime exposes the tools and fall back cleanly on auth/tool-access failure. Fall back to OpenAlex when Corbis is unavailable or returns no hit. You need at minimum the abstract; the introduction or first section is even better.
...
- **Corbis MCP** (skill `corbis`) — preferred for finance/econ papers. Read `process_log/corbis_status.json` first. Use the `search` capability to find the paper, then `batch_fetch` (or per-paper details) to get the abstract and (when available) full text when the runtime exposes those tools. Resolve every tool via `corbis_status["capabilities"]`; treat the map as default unverified tool names and fall back cleanly if auth/tool access fails.
