...
- **Corbis pass.** Read `process_log/corbis_status.json`. Corbis auth is OAuth/client-managed, so the preflight records `available: null`; use the `search` capability for direct prior-art lookup when the runtime exposes the tools (sortBy citation count, journals filter on the top finance/econ venues). If `synthesized_review` resolves to a tool and the runtime exposes it, use it for cross-subfield mechanism search; otherwise run multiple `search` calls with the abstract mechanism phrased differently each time. Resolve every tool name via `corbis_status["capabilities"]`; treat the map as default unverified tool names and fall back cleanly if auth/tool access fails.
