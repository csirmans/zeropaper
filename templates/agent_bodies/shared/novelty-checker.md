...
- **Corbis pass.** Read `process_log/corbis_status.json`. If `available` is `true` or `null` (OAuth/client-managed auth), use the `search` capability for direct prior-art lookup when the runtime exposes the tools (sortBy citation count, journals filter on the top finance/econ venues). If `synthesized_review` resolves to a non-null tool (Tier 2 / Enterprise), use it for cross-subfield mechanism search; otherwise run multiple `search` calls with the abstract mechanism phrased differently each time. Resolve every tool name via `corbis_status["capabilities"]`; when status is `null`, treat the map as default unverified tool names and fall back cleanly if auth/tool access fails. If `available` is `false`, skip the Corbis pass and rely on OpenAlex + WebSearch for this gate.
