...
0. **Optional Corbis enrichment pass.** Read `process_log/corbis_status.json`. Corbis auth is OAuth/client-managed, so the preflight records `available: null`; treat the capability map as default unverified tool names and try this pass only if the runtime exposes both the `search` and `batch_fetch` tools. If auth or tool access fails, fall back cleanly. Build the enrichment lookup as follows:
