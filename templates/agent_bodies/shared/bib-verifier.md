...
0. **Optional Corbis enrichment pass (only if Corbis is available or client-managed).** Read `process_log/corbis_status.json`. Skip this entire step if `available` is `false`, `capabilities["search"]` is `null`, or `capabilities["batch_fetch"]` is `null` (both capabilities are required — search to find candidates, batch_fetch to retrieve full content). If `available` is `null`, OAuth/client-managed auth may still expose Corbis tools; treat the capability map as default unverified tool names and try the pass only if the runtime exposes the tools, falling back cleanly on auth/tool-access failure. Otherwise, build the enrichment lookup as follows:
   - **For every bib entry**, regardless of whether it carries a DOI, call the `search` capability with the title (and authors as a fallback query if title-only is ambiguous). Corbis `id` fields are endpoint-specific (search can return OpenAlex-style IDs; top-cited can return Corbis UUIDs), but direct DOI input to `batch_fetch` is not reliable. Use the exact `id` returned by the validated Corbis hit; do not substitute the DOI from the bib.
   ...
   - Only matches passing the validation count as confirmed. Collect the confirmed **Corbis result IDs** and `batch_fetch` them in groups of ≤25 to retrieve abstract / full text.
