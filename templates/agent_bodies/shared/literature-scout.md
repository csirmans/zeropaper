...
- **Corbis MCP for domain-specialized search.** You have the `corbis` skill loaded — see it for full usage. Read `process_log/corbis_status.json` first. If `available` is `true` or `null` (OAuth/client-managed auth), run a Corbis pass in parallel with OpenAlex and WebSearch when the runtime exposes the tools: use the `search` capability for the core topic and `top_cited` for each target journal. If `available` is `false`, skip Corbis and run only OpenAlex + WebSearch. Resolve capabilities via `corbis_status.json["capabilities"]`; when status is `null`, treat the map as default unverified tool names and fall back cleanly if auth/tool access fails.
