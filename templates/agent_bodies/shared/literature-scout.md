...
- **Corbis MCP for domain-specialized search.** You have the `corbis` skill loaded — see it for full usage. Read `process_log/corbis_status.json` first and follow the shared state protocol: check `process_log/corbis_cache.jsonl`, reserve from budget scope `stage0_literature` before each MCP call, mark success with `--stage stage0_literature`, mark failures with the same stage, and cache reusable paper hits with `code/utils/corbis/state.py`. Corbis auth is OAuth/client-managed, so the preflight records `available: null`; run a Corbis pass in parallel with OpenAlex and WebSearch when the runtime exposes the tools and the stage budget allows it. Use the `search` capability for the core topic and selective `top_cited` calls for the most important target journals. Use `batch_fetch` only for top candidates likely to be reused downstream. Resolve capabilities via `corbis_status.json["capabilities"]`; treat the map as default unverified tool names and fall back cleanly if auth/tool access fails. If Corbis is disabled for this stage or budget is exhausted, continue with OpenAlex + WebSearch.
